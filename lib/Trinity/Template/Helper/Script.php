...
use Trinity\Template\Helper\Container;
use Trinity\Template\Helper\Exception;
use \Opt_Function;
...
class Script extends Container
{
	...
	/**
	 * Base URL for local scripts.
	 * @var String
	 */
	protected $_baseUrl;

	/**
	 * Sets base URL for local scripts.
	 *
	 ...
	 */
	public function prependFile($file, $local = true, $type = 'text/javascript', $options = array())
	{
		if((bool)$local)
		{
			$file = $this->_baseUrl.$file;
		}
		$options['src'] = $file;
		$options['type'] = $type;
		$row = array(
	 ...
	 * @param Array $options Extra options
	 */
	public function appendFile($file, $local = true, $type = 'text/javascript', $options = array())
	{
		if((bool)$local)
		{
			$file = $this->_baseUrl.$file;
	 ...
	 * @param Array $options Extra options
	 */
	public function offsetSetFile($offset, $file, $local = true, $type = 'text/javascript', $options = array())
	{
		if((bool)$local)
		{
			$file = $this->_baseUrl.$file;
